...
COUNT_JOINEVERYDAY	= 5	-- Ã¿Ìì¿ÉÒÔ²Î¼ÓÃÔ¹¬µÄ´ÎÊý
COUNT_JOINTOTAL		= 15	-- ×î´óÀÛ»ý´ÎÊý
...
function Maze:CheckTimeout()
	...
	if (self.m_LeftMinutes > 0) then
		...
		self:NotifyAllMates("Thêi gian thÝ luyÖn KiÕm Gia lÇn nµy ®· hÕt, chóc c¸c h¹ trong lÇn thÝ luyÖn sau nµy giµnh ®­îc thµnh tÝch tèt.")
		self:KickAll()
		self.m_GameTimerId = 0
		self:Destroy()
		return 0
...
function Room:Enter(player)
	if (self.m_Status == TASKSTAT_INIT) then
		...
		player:Msg2Player("C¸c ng­¬i ®· ®i vµo mét c¨n phßng thÇn bÝ, h·y Ên vµo Bia §¸ ë gi÷a c¨n phßng, sÏ cã nh÷ng sù m¹o hiÓm ®ang ®îi c¸c ng­¬i ë ®ã!")
	end
