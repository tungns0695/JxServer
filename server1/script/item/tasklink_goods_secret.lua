	...
	if (myTaskType == 4) then
		...
		if (nt_getTask(1031) == myMapID) then
			if (GetByte(nt_getTask(1032), 1) == 2) then
				...
				nt_setTask(5123,1)
				open_task_trace()
				Msg2Player("B¹n nhËn ®­îc mét tÊm" ..
				myMapName .. "§Şa §å chİ! HiÖn t¹i b¹n cã tæng céng " .. myMapNum .. " tÊm.");
		...
		Msg2Player("B¹n nhËn ®­îc mét m¶nh b¶n ®å S¬n Hµ X· T¾c! HiÖn t¹i b¹n cã tæng céng " ..
		nWorldMaps .. " m¶nh b¶n ®å S¬n Hµ X· T¾c.");
	else
		-- ¸øÍæ¼ÒÔö¼ÓÒ»¸öÉ½ºÓÉçğ¢Í¼²ĞÆ¬
		nWorldMaps = nWorldMaps + 1
		nt_setTask(1027, nWorldMaps)
