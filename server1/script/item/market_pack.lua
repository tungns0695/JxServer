...
function main(nItemIndex)
	...
	if P == 2333 then
        ...
        --xÝch thè---
        if P == 2328 then
		local tbAwardItem = {tbProp={0,10,5,7,0,0}, nBindState = nBindState}
		tbAwardTemplet:GiveAwardByList(tbAwardItem, "Më ra vËt phÈm mua ë Kú Tr©n C¸c");
		return 0;
	end
	if P == 2329 then
		local tbAwardItem = {tbProp={0,10,5,9,0,0,0}, nBindState = nBindState}
		tbAwardTemplet:GiveAwardByList(tbAwardItem, "Më ra vËt phÈm mua ë Kú Tr©n C¸c");
		return 0;
	end
	if P == 2330 then
		local tbAwardItem = {tbProp={0,10,5,8,0,0,0}, nBindState = nBindState}
		tbAwardTemplet:GiveAwardByList(tbAwardItem, "Më ra vËt phÈm mua ë Kú Tr©n C¸c");
		return 0;
	end
	if P == 2331 then
		local tbAwardItem = {tbProp={0,10,5,6,0,0,0}, nBindState = nBindState}
		tbAwardTemplet:GiveAwardByList(tbAwardItem, "Më ra vËt phÈm mua ë Kú Tr©n C¸c");
		return 0;
	end
	if P == 2332 then
		local tbAwardItem = {tbProp={0,10,5,10,0,0,0}, nBindState = nBindState}
		tbAwardTemplet:GiveAwardByList(tbAwardItem, "Më ra vËt phÈm mua ë Kú Tr©n C¸c");
		return 0;
	end
