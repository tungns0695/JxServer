...
local tbOpt =
	{
		...
		"Thay ®æi ngo¹i h×nh cho vò khÝ/feature_item",
...
function chose_typeweapon_final(nItemIndex,nGenre, nDetailType, nParticular,num,page)
	local tbOpt =  {
		"<dec><npc>§¹i hiÖp muèn chän vò khÝ nµo ?",
	};
	...
	if page ~= nil then
		...
		if nCount > 8 then
			...
			if page >= 1 then
				tinsert(tbOpt,format("Trang sau/#chose_typeweapon_final(%d,%d,%d,%d,%d,%d)",nItemIndex,nGenre, nDetailType, nParticular,num,page+1))
				tinsert(tbOpt,format("Trang tr­íc/#chose_typeweapon_final(%d,%d,%d,%d,%d,%d)",nItemIndex,nGenre, nDetailType, nParticular,num,page-1))
			end
			Msg2Player(""..page)
...
local tbOpt =
{
	{"Thªm Skills", g_AskClientStringEx, {"1200,20", 0, 256, "ID Skill", {AddSkill, {self}} }}, 
	{"Xãa Skills", g_AskClientStringEx, {"1200", 0, 300, "ID Skill", {DelSkill, {self}} }},
	{"Thªm Danh S¸ch Skills", g_AskClientStringEx, {"1200,1210", 0, 256, "ID Skill", {AddDSSkill, {self}} }}, 
...
function TaoItemID(szItem)
	...
	if (n ~= 1) then
	if (n < 3) then Talk(1, "", format("Sai Item Code. ThiÕu tr­êng yªu cÇu <color=yellow>%s<color>",n)); return end
	if (n > 3) then Talk(1, "", format("Sai Item Code. Qu¸ nhiÒu tr­êng d÷ liÖu <color=yellow>%s<color>",n)); return end
...
function change_mauchu ()
	local aryDescribe = {
		"<dec><npc>§¹i hiÖp muèn ®æi tr¹ng th¸i nµo ?",
	};
