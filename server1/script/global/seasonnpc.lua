...
function Task_NewVersionAward()
	...
	if (nNum - nCancelNum) == 30 then
		...
		tbAwardTemplet:GiveAwardByList({ nExp_tl = 30000000 },
			"PhÇn th­ëng kinh nghiÖm hoµn thµnh nhiÖm vô D· TÈu h»ng ngµy")
	end
	...
	if (nNum == 40 and nCancelNum) == 0 then
		...
		tbAwardTemplet:GiveAwardByList({ nExp_tl = 100000000 },
			"PhÇn th­ëng kinh nghiÖm khi hoµn thµnh nhiÖm vô D· TÈu h»ng ngµy")
	end
end
...
function main()
	...
	if (nDate >= 60808 and nDate <= 60815) then
		...
		Say(
		"MÊy n¨m nay chiÕn tranh triÒn miªn, d©n t×nh thèng khæ. Ph­¬ng tr­îng ThiÕu L©m tù muèn tæ chøc thä trai mõng LÔ Vu Lan ®Ó mäi ng­êi h­ëng chót gi©y phót yªn b×nh",
...
function menglanjie()
	if (GetExtPoint(0) <= 0 or GetLevel() < 30) then
		Say("Xin lçi! ChØ cã ng­êi ch¬i tõ cÊp <color=red>30, ®· n¹p thÎ<color> míi cã thÓ tham gia ho¹t ®éng nµy",
			0)
		return
	end;
	local tab_Content = {
...
MAKING_COUNT = 9;
tab_Flower = {
	{ 1126, "Kim", 1131, 1760, 1761 }, { 1127, "Méc", 1132, 1762, 1763 }, { 1128, "Thñy ", 1133, 1764, 1765 },
	{ 1129, "Háa", 1134, 1766, 1767 }, { 1130, "Thæ ", 1135, 1768, 1769 }
}
...
function process(nIdx)
	...
	tab_Flower[nIdx][2] ..
	" Liªn Hoa<color> cÇn cã 9 <color=yellow>" .. tab_Flower[nIdx][2] .. "Liªn Hoa<color>. Ng­¬i x¸c ®Şnh lµm chø?",
		2, "§óng! Xin l·o bèi træ tµi!/#make_round(" .. nIndex .. ")", "Ta sÏ quay l¹i sau!/Task_Wait");
end;

function make_round(nIdx)
	if (GetCash() < MAKING_COST) then
		Say("Xin t×m ®ñ 10000 l­îng råi h·y quay l¹i! Ta ë ®©y chê!", 1,
			"§­îc th«i! Ta ®i lÊy thªm tiÒn!/Task_Wait");
		return
	end;
	...
	if (nCount < 9) then
		...
		tab_Flower[nIdx][2] ..
		" Liªn Hoa<color> th× ta kh«ng thÓ gióp ng­¬i kÕt vßng " .. tab_Flower[nIdx][2] .. " Liªn Hoa hoµn", 1,
	...
	Say("Vßng " .. tab_Flower[nIdx][2] .. " Liªn Hoa cña ng­¬i ®· kÕt xong. H·y mang nã ®Õn chç LÔ Quan ®i!",
		1, "NhËn vßng hoa/Task_Wait");
...
function tasklink_entence()
	...
	if (tl_gettaskcourse() == 0) then
		...
		" VŞ nµy" ..
		GetPlayerSex() ..
		"xem ra ®· b«n ba giang hå ®­îc mét thêi gian dµi råi nhØ, cã muèn tham gia kh¶o nghiÖm nhiÖm vô liªn tôc cña ta kh«ng?",
			3, "§­îc th«i! Ta kh«ng tin cã nhiÖm vô nµo lµm khã dÔ ®­îc ta/Task_Confirm",
	...
	if (tl_gettaskcourse() == 3) then
		...
		(400 - GetTask(ID_TASKLINK_LIMITNUM)) .. "<color>, ng­¬i cã muèn lµm n÷a kh«ng?", 2,
...
function Task_MainDialog()
	...
	if myTaskTotalNum == 0 or myTaskTotalNum == nil then
		...
		myTitleText = "D· TÈu:<enter><enter> §©y lµ <color=green>" ..
		myTaskTotalNum .. "<color> nhiÖm vô thø, " .. myTaskInfo;
	end;
...
function Task_TotalCancel()
	GiveItemUI("Giao thñy tinh cho D· TÈu",
		"NÕu ng­¬i ®· quyÕt hñy bá chuçi nhiÖm vô, ta còng kh«ng ng¨n c¶n, h·y ®Æt vµo viªn Lôc Thñy Tinh!",
		"Task_TotalCancel_Main", "Task_Wait");
end;
...
function Task_TotalCancel_Main(nCount)
	...
	if nCount ~= 1 then
		Say("D· TÈu: Ng­¬i ®­a g× cho ta vËy? Ta chØ cÇn <color=yellow>1 viªn Lôc Thñy Tinh<color> th«i!",
			0);
		return
	end;
...
function Task_ProcessInfo()
	...
	myCountTimes .. "<color> sè lÇn nhiÖm vô ta giao cho, cÇn cè g¾ng h¬n nhĞ!", 0);
end
...
function Task_Accept()
	...
	if (myTaskType == 1) then
		GiveItemUI("NhiÖm vô t×m vËt phÈm", " ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
			"Task_Accept_01", "Task_Wait");
	elseif (myTaskType == 2) then
		GiveItemUI("NhiÖm vô mua vËt phÈm", " ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
			"Task_Accept_02", "Task_Wait");
	elseif (myTaskType == 3) then
		GiveItemUI("NhiÖm vô hiÓn thŞ vËt phÈm", " ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
			"Task_Accept_03", "Task_Wait");
...
function Task_Accept_01(nCount)
	...
	if (nCount > 1) then
		Say(" VŞ nµy" .. GetPlayerSex() .. ", Ng­¬i bá nhiÒu ®å v« nh­ vËy xem tíi ta hoa c¶ m¾t, tõ tõ th«i nµo!",
			0);
		return 0
	elseif (nCount == 0) then
		Say(" VŞ nµy" .. GetPlayerSex() ..
		", ng­¬i cã thËt ®· bá vµo thø ta cÇn kh«ng? Kh«ng ph¶i l·o phu hoa m¾t chø?", 0);
		return 0
	end
...
function Task_Cancel(nType)
	...
	if (nType == 2) then
		...
		-- else
		-- 	Say(" ng­¬i cã ®óng ®· mang <color=yellow>100<color> m¶nh s¬n Hµ X· T¾c kh«ng? Ta cã nh×n lÇm kh«ng vËy?",
		-- 		0);
		-- 	return
		-- end;
	end;
...
function Task_GiveAward()
	...
	if (nTotalTaskNum ~= 0 and mod(nTotalTaskNum, 10) == 0 and GetTask(TKS_TASKLINK_SPITEM) ~= nTotalTaskNum) then
		...
		Msg2Player(format("Chóc mõng ®¹i hiÖp ®· hoµn thµnh liªn tiÕp %d nhiÖm vô D· TÈu, nhËn ®­îc phÇn th­ëng %s!",
			10, "B¶o r­¬ng thÇn bİ cña D· TÈu"));
	end
	...
	for i = 1, 3 do
		...
		elseif (myAward[i][1] == 3) then
			myGoodsText = myAward[i][3] ..
			"," .. myAward[i][4] .. "," .. myAward[i][5] .. "," .. myAward[i][6] ..
			"," .. myAward[i][7] .. "," .. myAward[i][8]
			ShowText[i] = "NhËn ®­îc" .. myAward[i][9] .. "/5" .. "/" .. myGoodsText .. "/mySG"
		elseif (myAward[i][1] == 4) then
			if (myAward[i][10] == 1) then
				myGoodsText = myAward[i][3] ..
				"," .. myAward[i][4] .. "," .. myAward[i][5] ..
				"," .. myAward[i][6] .. "," .. myAward[i][7] .. "," .. myAward[i][8]
	...
	Prise("Ng­¬i vÊt v¶ qu¸, xin mêi vŞ " .. GetPlayerSex() .. "Chän mãn m×nh thİch ®i!", ShowText[1],
		ShowText[2], ShowText[3]);
...
function mySG(myQuality, myGenre, myDetail, myParticular, myLevel, myFive)
	...
	if (myQuality == 0) then
		...
		AddGlobalNews("Ng­êi ch¬i " ..
		GetName() .. " ®· hoµn thµnh nhiÖm vô D· TÈu nªn ®· nhËn ®­îc 1 trang bŞ Hoµng Kim!!!");
		WriteLog(" [Ghi nhí nhËn phÇn th­ëng]" ..
		date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
		": Tµi kho¶n" ..
		GetAccount() ..
		", nh©n vËt" ..
		GetName() .. "Trong nhiÖm vô liªn tiÕp nhËn ®­îc phÇn th­ëng lµ trang bŞ Hoµng Kim sè thø tù lµ:" ..
		myGenre)
	end
...
function SelectAward_Cancel(nAward)
	...
	WriteLog(" [Ghi nhí nhËn phÇn th­ëng]" ..
		...
		" [Tµi kho¶n]" .. GetAccount() ..
		" [Nh©n vËt]" .. GetName() ..
		"B¹n nhËn ®­îc c¬ héi hñy bá nhiÖm vô, hiÖn t¹i cßn cã thÓ c¬ héi hñy <" .. myCancel ..
		"> lÇn.");
...
function PayPlayerLinkAward()
	local myGolden = { 2, 6, 11, 16, 21, 22, 26, 31, 39, 40, 42, 46, 51, 54, 56, 60, 61, 67, 71, 76, 81, 87, 92, 94, 96,
		101, 106, 107, 115, 119, 121, 122, 126, 132, 136, 144, 145, 146 }                                                                       -- 20 »·½±Àø»Æ½ğ×°±¸µÄ±àºÅ
	local nGoldenID = 0
...
function _WriteCancelLog(nType, nTime, nCancel)
	if nType == 1 then
		WriteLog(" [Hñy bá kû lôc nhiÖm vô liªn tiÕp]" ..
		date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
		": Tµi kho¶n" ..
		GetAccount() ..
		", nh©n vËt" ..
		GetName() .. "T¹i nhiÖm vô thø " .. nTime .. "Hñy bá c¬ héi cßn d­ " ..
		nCancel .. "giê hñy bá mét lÇn nhiÖm vô.");
	else
		WriteLog(" [Hñy bá kû lôc nhiÖm vô liªn tiÕp]" ..
		date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
		": Tµi kho¶n" ..
		GetAccount() ..
		", nh©n vËt" .. GetName() .. "T¹i nhiÖm vô thø " .. nTime .. " dïng 100 tÊm s¬n Hµ X· T¾c hñy bá nhiÖm vô.");
	end;
end;
